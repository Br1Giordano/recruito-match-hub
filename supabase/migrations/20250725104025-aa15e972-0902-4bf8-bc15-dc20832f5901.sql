-- Fix the ambiguous column references and continue RLS implementation
...
FOR SELECT 
USING (
  auth.uid() = proposals.user_id OR 
  auth.uid() IN (
    SELECT jo.user_id FROM public.job_offers jo WHERE jo.id = proposals.job_offer_id
  )
);
...
ON public.proposals 
FOR INSERT 
WITH CHECK (auth.uid() = proposals.user_id);

CREATE POLICY "Users can update proposals they created" 
ON public.proposals 
FOR UPDATE 
USING (auth.uid() = proposals.user_id);
...
USING (
  auth.uid() IN (
    SELECT p.user_id FROM public.proposals p WHERE p.id = proposal_responses.proposal_id
  ) OR
  auth.uid() IN (
    SELECT jo.user_id FROM public.job_offers jo 
